...
from quant.services.universe.universe_builder_service import UniverseBuilderService


class BuildUniverseSnapshotJob(BaseJob):
    job_name = "build_universe_snapshot"
    ...
    def execute(self, session: Session, target_date: str | None, force: bool, run_mode: str, attempt_no: int) -> JobResult:
        ...
        service = UniverseBuilderService(session=session)
        result = service.build(target_date=target_date, universe_name=settings.default_universe_name)
        ...
        return JobResult(
            ...
            metadata={"run_mode": run_mode, "attempt_no": attempt_no, "universe_name": result.universe_name},
        )
