...
class KrxClient:
    """KRX transport scaffold.

    This client intentionally only provides generic transport behavior.
    Endpoint-specific response mapping is handled in provider adapters.
    """
    ...
    def call(self, request: KrxRequest) -> list[dict[str, Any]]:
        ...
        try:
            ...
            raise ProviderNotImplementedError(
                f"KRX request failed with HTTP {exc.code} for api_name={request.api_name}"
            ) from exc
        except URLError as exc:
            raise ProviderNotImplementedError(
                f"KRX request failed for api_name={request.api_name}: {exc.reason}"
            ) from exc
        ...
        raise ProviderNotImplementedError(
            "KRX response format is unsupported. Expected list[dict] or {'data': list[dict]}."
        )
